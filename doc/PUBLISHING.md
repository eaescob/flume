...
4. Create a git tag: `git tag v1.2.5 && git push origin v1.2.5`
...
1. Build the release and create a tarball of the source:
   ```sh
   git archive --format=tar.gz --prefix=flume-1.2.5/ v1.2.5 > flume-1.2.5.tar.gz
   ```

2. Calculate the SHA256:
   ```sh
   shasum -a 256 flume-1.2.5.tar.gz
   ```
...
4. Copy the updated formula to the tap repo and push:
   ...
   cd ../homebrew-tap && git add -A && git commit -m "flume 1.2.5" && git push
   ```
...
4. Push to AUR:
   ...
   git add -A && git commit -m "Update to 1.2.5" && git push
   ```
